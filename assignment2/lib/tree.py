...
class DecisionTree():
    ...
    def _build(self, train, depth=1):
        ...
        # information gain
        _gain = lambda groups: -sum([self.metric(g)*len(g)/len(list(chain(*groups))) for g in groups])
        ...
        i, j = max(zip(self.features, m), key=lambda t: _gain(_split(t[0], train[t[1]][t[0]])))
        ...
        elif depth >= self.max_depth or -_gain([l, r]) < self.min_gain:
    ...
    def fit(self, X, y, max_depth=32, min_size=0, min_gain=.005):
